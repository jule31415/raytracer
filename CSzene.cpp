...
struct MouseParams
{
    ...
    float b[54]={0};
    float rough[54]={0};
    float rsel=0, gsel=0.5, bsel=0.5;
    float roughsel=0;
    int npoints=0;
    bool randomcol=true;
    bool skybox=false;
...
if  ( event == EVENT_LBUTTONDOWN )
     {
         ...
         Mat & draw2 = mp->draw2;
         if(hi>20){
         if (mp->randomcol==true){
         mp->rsel=0;
         mp->gsel=0.5+0.5*sin(mp->npoints+4);
         mp->bsel=0.5+0.5*sin(mp->npoints+6);
         };
         mp->xwords[mp->npoints]=lo;
         mp->ywords[mp->npoints]=hi;
         mp->r[mp->npoints]=mp->rsel;
         mp->g[mp->npoints]=mp->gsel;
         mp->b[mp->npoints]=mp->bsel;
         mp->rough[mp->npoints]=mp->roughsel;//0.1+0.1*sin(3*mp->npoints);
         circle( draw2, Point(lo,hi), 10, Scalar(255*mp->r[mp->npoints],255*mp->g[mp->npoints],255*mp->b[mp->npoints]), -1, 8);
         mp->npoints=mp->npoints+1;
         imshow("Create Spheres by clicking", draw2);}
         else{
         mp->roughsel=lo/250.0*0.3;
         };

     }
}

void CallBackFuncCol(int event, int lo, int hi, int flags, void* data){
...
void CSzene::CreateNewSpheres(){
	...
	formenliste[3] = &*p;
	namedWindow("Create Spheres by clicking", WINDOW_AUTOSIZE);
    MouseParams mopa;
    mopa.draw2= Mat::zeros(250, 250, CV_8UC3);
    rectangle(mopa.draw2, Point(1,1), Point(249,20),Scalar(200,200,200), -1, 8);
    putText(mopa.draw2, "min", Point(5, 10),0, 0.4,Scalar(0, 0,0), 1);
    putText(mopa.draw2, "roughnes", Point(100, 10),0, 0.4,Scalar(0, 0,0), 1);
    putText(mopa.draw2, "max", Point(220, 10),0, 0.4,Scalar(0, 0,0), 1);

    namedWindow("select color", WINDOW_AUTOSIZE);
    Mat selectcol=Mat::zeros(310, 255, CV_8UC3);
    for (int ix=20;ix<254;ix++){
    for (int iy=0;iy<254;iy++){
	rectangle(selectcol, Point(ix, iy), Point(ix+1, iy+1),Scalar(iy,ix,255-ix), -1, 8);
	};
	};
    putText(selectcol, "r", Point(5, 50),0, 0.4,Scalar(250,250,250), 1);
    putText(selectcol, "a", Point(5, 80),0, 0.4,Scalar(250,250,250), 1);
    putText(selectcol, "n", Point(5, 110),0, 0.4,Scalar(250,250,250), 1);
    putText(selectcol, "d", Point(5, 140),0, 0.4,Scalar(250,250,250), 1);
    rectangle(selectcol, Point(0,256), Point(126, 310),Scalar(300,300,300), -1, 8);
    putText(selectcol, "no skybox", Point(20, 290),0, 0.4,Scalar(10,10,10), 1);
    putText(selectcol, "skybox", Point(190, 290),0, 0.4,Scalar(250,250,250), 1);
    imshow("Create Spheres by clicking",mopa.draw2);
    imshow("select color",selectcol);
	setMouseCallback("Create Spheres by clicking", CallBackFuncSpheres, &mopa); //null //&myszene
    setMouseCallback("select color", CallBackFuncCol, &mopa);
	char key='a';
	while (key=='a'){
    ...
    for (int i = 0; i < 52; i++) {
        p = new Kugel( 10*mopa.xwords[i], 10*mopa.ywords[i] , 50, 100, 0.2+0.2*sin(i), 5,mopa.r[i],mopa.g[i],mopa.b[i],mopa.rough[i]);
		//p = new Kugel( *(xwort+i), *(ywort+i) , 50, 10, 0, 5,0,0,0);
		formenliste[4+i] = &*(p); //9+i
		formencount=formencount+1;
	}
	skybox_bool=mopa.skybox;
}
...
//#pragma omp target
    Mat imagesky = imread("spaceship.jpg");
			...
			for (int ireflexion=0;ireflexion<3;ireflexion++){
                ...
                if (hitsobject==true){
                    ...
                    fa_b=fa_b+(alb_b*fa1);
                        ...
                        hitsobject=false;
                        myray[ireflexion]->calc_uv(&u,&v);
                        if (skybox_bool==true){
                        fa_r=fa_r+imagesky.at<cv::Vec3b>(int(imagesky.rows/3.14*u),int(imagesky.cols*(v+3.14)/2/3.14))[0]/5.0;
                        fa_g=fa_g+imagesky.at<cv::Vec3b>(int(imagesky.rows/3.14*u),int(imagesky.cols*(v+3.14)/2/3.14))[1]/5.0;
                        fa_b=fa_b+imagesky.at<cv::Vec3b>(int(imagesky.rows/3.14*u),int(imagesky.cols*(v+3.14)/2/3.14))[2]/5.0;};
                        //cout<<"v="<<v;
                    };
                };
	...
	putText(draw1, "fast", Point(5, 480),0, 0.4,Scalar(0, 0,0), 1);
	rectangle(draw1, Point(1, 550), Point(50, 600),Scalar(150, 50, 150), -1, 8);
	putText(draw1, "higher", Point(5, 565),0, 0.3,Scalar(0, 0,0), 1);
	putText(draw1, "resolution", Point(5, 580),0, 0.3,Scalar(0, 0,0), 1);
	rectangle(draw1, Point(1, 650), Point(50, 700),Scalar(150, 50, 150), -1, 8);
	putText(draw1, "render ", Point(5, 665),0, 0.3,Scalar(0, 0,0), 1);
	putText(draw1, "20x", Point(5, 680),0, 0.3,Scalar(0, 0,0), 1);
