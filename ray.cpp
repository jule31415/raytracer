...
void ray::calc_uv(float *u, float *v){
    *u=atan2(sqrt(float(pow(sx-px,2)+pow(sy-py,2))),-(pz-sz)*1.0);//(ix-sx)/sqrt(float(pow(sx-ix,2)+pow(sy-iy,2)+pow(sz-iz,2)));
    *v=atan2((py-sy)*1.0,(px-sx)*1.0);//(iy-sy)/sqrt(float(pow(sx-ix,2)+pow(sy-iy,2)+pow(sz-iz,2)));
}